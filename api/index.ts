...
// In-Memory Database Fallback to absolutely guarantee 500-free runtime on read-only filesystems
...
function readDB() {
  ...
  // If we already had to fall back to memory or want to keep it consistent
  if (inMemoryDB) {
    return inMemoryDB;
  }
  ...
  // If file doesn't exist or is corrupt, initialize the memory cache
  if (!inMemoryDB) {
    inMemoryDB = defaults;
  }
  return inMemoryDB;
...
function writeDB(data: { scores: number[]; opinions: string[]; names: string[] }) {
  // Always update our memory model first
  inMemoryDB = data;
...
// API Routes (supports multiple variants in case Vercel rewrites or strips the path prefix)
app.get(["/api/records", "/records"], (req, res) => {
  try {
    res.json(readDB());
  } catch (err: any) {
    console.error("Critical error in /api/records GET:", err);
    res.status(200).json(inMemoryDB || getDefaultData()); // Ultimate fail-safe
  }
});
...
예시:
"오늘 마음이 참 무겁고 피겁했겠구나.. 잠시 따뜻한 물 한 잔 마시면서 긴장을 풀어 보렴. 선생님이 항상 이곳에서 네 이야기를 귀담아듣고 응원해 줄 테니까 힘내거라.
[힘들고 우울한 마음]"
`;
